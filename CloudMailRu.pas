...
var
	...
	OperationStatus, OperationResult: integer;
begin
	URL := 'https://cloud.mail.ru/api/v2/folder?sort={%22type%22%3A%22name%22%2C%22order%22%3A%22asc%22}&offset=0&limit=10000&home=' + PathToUrl(path) + 	Progress := false;
...
begin
	...
	if self.HTTPPost(URL, '&home=' + PathToUrl(OldName) + '&name=' + PathToUrl(NewName) + self.united_params;
self.united_params, JSON) then
	begin //Парсим ответ
		OperationResult :=self.fromJSON_OperationResult(JSON, OperationStatus);
		case OperationResult of
			CLOUD_OPERATION_OK:
