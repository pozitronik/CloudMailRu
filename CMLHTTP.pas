...
//		Property ConnectTimeoutValue: integer read Settings.ConnectTimeout;
		...
		function PostForm(URL: WideString; PostDataString: WideString; var Answer: WideString; ContentType: WideString = 'application/x-www-form-urlencoded'; LogErrors: Boolean = true; ProgressEnabled: Boolean = true): Boolean; //ѕостинг данных с возможным получением ответа.
		function PostMultipart(URL: WideString; Params: TDictionary<WideString, WideString>; var Answer: WideString): Boolean;
		function PostFile(URL: WideString; FileName: WideString; FileStream: TStream; var Answer: WideString): integer; overload; //ѕостинг потока данных как файла
...
begin
	...
	try
		if ProgressEnabled then
			HTTP.OnWork := self.Progress//¬ызов прогресса ведЄт к возможности отменить получение списка каталогов и других операций, поэтому он нужен не всегда
		else
			HTTP.OnWork := nil;
		Answer := HTTP.Get(URL);
					...
					begin
						case HTTP.ResponseCode of
							HTTP_ERROR_BAD_REQUEST, HTTP_ERROR_OVERQUOTA: //recoverable errors
								begin
									//Answer := (E as EIdHTTPProtocolException).ErrorMessage; //TODO: нужно протестировать, наверн€ка тут не json
								end;
						end;
					end;
			end;
...
{TODO: Ѕазовый метод постинга должен работать с потоком}
...
begin
	...
	PostResult := self.Post(URL, PostData, ResultStream, true, ContentType, LogErrors, ProgressEnabled); {TODO: будут логироватьс€ все ошибки, в т.ч. разбираемые уровнем выше => передавать обработку ошибок вышесто€щему парсеру}
...
begin
	...
	if AWorkMode = wmRead then
		...
		ContentLength := HTTP.Request.ContentLength; //—читаем размер обработанных данных в зависимости от того, скачивание это или загрузка
