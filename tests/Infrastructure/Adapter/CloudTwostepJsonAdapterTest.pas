unit CloudTwostepJsonAdapterTest;
...
type
	[TestFixture]
	TCloudTwostepJsonAdapterTest = class
	private
		const
			{Full two-step data}
...
procedure TCloudTwostepJsonAdapterTest.TestParse_Full_ReturnsTrue;
var
	Twostep: TCloudTwostep;
begin
	Assert.IsTrue(TCloudTwostepJsonAdapter.Parse(JSON_TWOSTEP_FULL, Twostep));
end;

procedure TCloudTwostepJsonAdapterTest.TestParse_Full_ParsesFormName;
var
	Twostep: TCloudTwostep;
begin
	TCloudTwostepJsonAdapter.Parse(JSON_TWOSTEP_FULL, Twostep);
	Assert.AreEqual('auth_form', Twostep.form_name);
end;

procedure TCloudTwostepJsonAdapterTest.TestParse_Full_ParsesAuthHost;
var
	Twostep: TCloudTwostep;
begin
	TCloudTwostepJsonAdapter.Parse(JSON_TWOSTEP_FULL, Twostep);
...
procedure TCloudTwostepJsonAdapterTest.TestParse_Full_ParsesSecstepPhone;
var
	Twostep: TCloudTwostep;
begin
	TCloudTwostepJsonAdapter.Parse(JSON_TWOSTEP_FULL, Twostep);
...
procedure TCloudTwostepJsonAdapterTest.TestParse_Full_ParsesTimeout;
var
	Twostep: TCloudTwostep;
begin
	TCloudTwostepJsonAdapter.Parse(JSON_TWOSTEP_FULL, Twostep);
...
procedure TCloudTwostepJsonAdapterTest.TestParse_Full_ParsesCsrf;
var
	Twostep: TCloudTwostep;
begin
	TCloudTwostepJsonAdapter.Parse(JSON_TWOSTEP_FULL, Twostep);
...
procedure TCloudTwostepJsonAdapterTest.TestParse_AuthApp_SetsAuthAppUsed;
var
	Twostep: TCloudTwostep;
begin
	TCloudTwostepJsonAdapter.Parse(JSON_TWOSTEP_AUTH_APP, Twostep);
	Assert.AreEqual(Int64(AUTH_APP_USED), Twostep.secstep_timeout);
end;

procedure TCloudTwostepJsonAdapterTest.TestParse_Partial_ReturnsTrue;
var
	Twostep: TCloudTwostep;
begin
	Assert.IsTrue(TCloudTwostepJsonAdapter.Parse(JSON_TWOSTEP_PARTIAL, Twostep));
	Assert.AreEqual('auth_form', Twostep.form_name);
	Assert.AreEqual('abc123', Twostep.csrf);
end;

procedure TCloudTwostepJsonAdapterTest.TestParse_InvalidJSON_ReturnsFalse;
var
	Twostep: TCloudTwostep;
begin
	Assert.IsFalse(TCloudTwostepJsonAdapter.Parse(JSON_INVALID, Twostep));
end;

procedure TCloudTwostepJsonAdapterTest.TestParse_EmptyString_ReturnsFalse;
var
	Twostep: TCloudTwostep;
begin
	Assert.IsFalse(TCloudTwostepJsonAdapter.Parse(JSON_EMPTY, Twostep));
end;

initialization

TDUnitX.RegisterTestFixture(TCloudTwostepJsonAdapterTest);
